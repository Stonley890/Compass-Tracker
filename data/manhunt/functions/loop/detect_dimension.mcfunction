...
execute if score .compass-feedback manhunt-options matches 1 as @a[team=Hunter,nbt={SelectedItem: {id: "minecraft:compass"}}] at @s unless entity @a[team=Runner,gamemode=!spectator,distance=0..] if score .endgame-active manhunt-options matches 1 run title @s actionbar [{"text":"Compass Refreshed","color":"yellow"}]
